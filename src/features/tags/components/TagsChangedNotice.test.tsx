...
vi.mock("../api/customTagsApi", () => ({ customTagsApi: { poolAll: vi.fn(async () => []) } }));
...
it("显式刷新请求新候选，但不重写旧 ID 或重试原结果查询", async () => {
  ...
  expect(customTagsApi.poolAll).not.toHaveBeenCalled();
  fireEvent.click(screen.getByRole("button", { name: "刷新标签候选" }));
  await waitFor(() => expect(customTagsApi.poolAll).toHaveBeenCalledWith({ q: "", selectable: false, include_deleted: false }, expect.any(AbortSignal)));
