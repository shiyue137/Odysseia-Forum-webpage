...
export function TargetTagSection({ target, ownerId, children, onSearch, targetLabel = target.type === "thread" ? "帖子" : "书单" }: { target: TagTarget; ownerId?: string | null; children?: ReactNode; onSearch: (tag: { id: string; name: string }) => void; targetLabel?: string }) {
  ...
  const canEdit = !!user?.id && (String(user.id) === ownerId || role.data?.is_management_member === true);
...
function TagSelectionDialog({ target, targetLabel, initial, canEdit, onClose, onSaved }: {
  ...
  const [selected, setSelected] = useState(() => new Map(canEdit ? initial.tags.filter((tag) => tag.source === "custom").map((tag) => [tag.id, tag.name]) : []));
  const [baseline, setBaseline] = useState(initial);
  const [conflict, setConflict] = useState<TargetTags | null>(null);
  const [conflictPending, setConflictPending] = useState(false);
  const [failures, setFailures] = useState<string[]>([]);
  const existingIds = initial.tags.map((tag) => tag.id);
  useEffect(() => {
  ...
  const submit = useMutation({
    mutationFn: async () => {
      setFailures([]);
      if (canEdit) {
        try { await customTagsApi.replace(target, baseline.version, [...selected.keys()]); }
        catch (error) {
  ...
  function toggle(tag: PoolTag) {
    if (!canEdit && existingIds.includes(tag.id)) return;
    setSelected((current) => {
  ...
  return createPortal(<dialog ref={ref} aria-labelledby="tag-selection-title"
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="min-h-0 flex-1 px-4 py-4 sm:px-6">
        <LiveTagPool selectedIds={[...selected.keys()]} disabledIds={canEdit ? undefined : existingIds} onToggle={toggle} busy={submit.isPending} />
      </div>
      <footer className="max-h-[35dvh] shrink-0 overflow-y-auto border-t border-(--od-border) p-4">
        ...
        <p className="mb-2 text-xs text-(--od-text-tertiary)">已选 {selected.size} 个{canEdit && target.type === "thread" && ` · 原生标签 ${baseline.tags.filter((tag) => tag.source === "discord").length} 个（只读）`}{canEdit && " · 合计上限 12 个"}</p>
        {conflictPending && <div role="alert" className="mb-3 text-sm">
          <p>标签已被修改。本地选择已保留，请核对最新集合后确认。</p>
          {conflict ? <><p className="my-2">最新标签：{conflict.tags.map((tag) => tag.name).join("、") || "无"}</p><button className={action} onClick={() => { setBaseline(conflict); setConflict(null); setConflictPending(false); submit.reset(); }}>已核对，以当前选择替换</button></> :
